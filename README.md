...
| 命令 | 说明 |
|------|------|
| `submit_project` | 提交新研究项目 (含 topic) |
| `add_lobster` | 动态添加龙虾到指定层 |
| `remove_lobster` | 移除龙虾 |
| `stop_agent` | 停止指定 Agent |
| `list_agents` | 列出所有 Agent 状态 |
| `get_queues` | 获取队列状态 |
| `get_shared_results` | 获取共享结果摘要 |

## 相对于原版 AutoResearchClaw 的改动

| 改动 | 说明 |
|------|------|
| `--to-stage` 支持 | 龙虾在指定 stage 停止,支持分层执行 |
| S10 CODEBASE_SEARCH | 搜索 + 下载可复用 GitHub 代码库 |
| S12 SANITY_CHECK | 代码冒烟测试 + GPU 验证 + LLM 自动修复 |
| S18 KNOWLEDGE_SUMMARY | 结论归纳写入共享知识库 |
| `coding_model` 配置 | S11 可用独立 Coding 模型 |
| DeepSeek 兼容 | 修复 json_mode 的 HTTP 400 |
| 本地路径注入 | datasets / checkpoints / codebases 注入 prompt |
| 共享结果注册 | S16 注册 + S14 LLM 语义匹配查询 |
